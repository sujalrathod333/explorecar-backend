...
import carsData from "../assets/FRONTEND/carsData";
import { carDetailStyles } from "../assets/FRONTEND/dummyStyles";
...
const buildImageSrc = (image) => {
  if (!image) return "https://placehold.co/800x450/e5e7eb/6b7280?text=Car+Image";
  if (Array.isArray(image)) image = image[0];
  if (!image || typeof image !== "string")
    return "https://placehold.co/800x450/e5e7eb/6b7280?text=Car+Image";
  const t = image.trim();
  if (!t) return "https://placehold.co/800x450/e5e7eb/6b7280?text=Car+Image";
  if (t.startsWith("http://") || t.startsWith("https://")) return t;
  if (t.startsWith("/")) return `${API_BASE}${t}`;
  // Remove any leading 'uploads/' to avoid duplication
  const cleanPath = t.replace(/^uploads\//, '');
  return `${API_BASE}/uploads/${cleanPath}`;
};

const handleImageError = (
  e,
  fallback = "https://placehold.co/800x450/e5e7eb/6b7280?text=Car+Image"
) => {
  const img = e?.target;
  if (!img) return;
  img.onerror = null;
  ...
  img.onerror = () => {
    img.onerror = null;
    img.src = "https://placehold.co/800x450/e5e7eb/6b7280?text=No+Image";
  };
