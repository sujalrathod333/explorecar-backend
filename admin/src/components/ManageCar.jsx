...
import { styles, toastStyles } from "../assets/dummyStyles";
...
const StatCard = ({ title, value, icon: Icon, className = "" }) => {
  return (
    <div
      className={`${styles.gradientOrange} ${styles.rounded2xl} ${styles.borderGray} ${styles.statCard}
      ${styles.borderHoverOrange} ${className}
      `}
    >
...
const CarCard = ({ car, onEdit, onDelete }) => {
  ...
  return (
    <div
      className={`${styles.gradientGray} ${styles.rounded2xl} ${styles.borderGray} ${styles.carCard}
    ${styles.borderHoverOrange} p-0 h-full flex flex-col`}
    >
      <div className="relative">
        ...
        <div className="absolute top-4 right-4">
          <span
            className={`px-3 py-1 text-xs font-semibold rounded-full ${getStatusStyle(car.status)}`}
          >
...
const NoCarView = ({ onResetFilter }) => (
  <div className={`${styles.gradientGray} ${styles.noCarsContainer}`}>
    <div
      className="mx-auto w-24 h-24 rounded-full bg-gradient-to-r from-orange-900/30 to-amber-900/30 
  flex items-center justify-center mb-6"
    >
      <div
        className="bg-gradient-to-r from-orange-700 to-amber-700 w-16 h-16 flex rounded-full
  ...
  const handleDelete = async (identifier) => {
    ...
    try {
      if(!car._id) {
        ...
        toast.success("Car Remove Locally")
        return;
      }
